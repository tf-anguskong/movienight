...
async function doRetune(room, io) {
  ...
  try {
    ...
    await prewarmManifest(room.id, ratingKey, true, room.liveTvChannelId, subKey, sessionKey).catch(() => {});
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('select-livetv-channel', async ({ channel, channelTitle, channelId }) => {
      ...
      try {
        ...
        await prewarmManifest(room.id, ratingKey, true, channelId, subKey, sessionKey).catch(() => {});
